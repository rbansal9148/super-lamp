...
KEY PROPERTIES: pure bash + kubectl + Python + VictoriaMetrics PromQL; no policy engine, no scoring tool (Polaris/kube-score/Kyverno were evaluated and REJECTED — they break byte-stable diffable output + owned-ns scoping). --json output is jq-built (correct escaping). Thresholds are hand-coded constants in thresholds.sh, overridable by env. GENUINE REMAINING FRONTIER: NO CPU sizing/throttling (throttle metric not even scraped); MAX-only memory usage (no p95, so chronic over-provisioning hidden behind single spikes); NO image CVE scan (trivy not installed — the one tool that cleared the adoption bar, as opt-in --deep); image-currency tool not wired into the punch list; findings still point-in-time — NO result storage, NO trend/history, NO CI/PR gate, NO fix-landing tracking.`
