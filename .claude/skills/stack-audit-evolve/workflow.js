...
02-image-pins: flags running containers whose image has no @sha256 digest (MED for :latest/no-tag, LOW for version-tag-no-digest). Pin-by-digest only; NO vuln scan, NO signature/SBOM. CURRENCY is now covered by the in-repo Rust binary tools/image-currency/ (floating-tag UPDATE + version-pin DRIFT via generic Www-Authenticate, report-only with --apply/--repin-drift), wired into audit.sh as the opt-in \`--updates\` section (Jun 2026) — do NOT re-propose digest-currency/drift detection or Renovate/argocd-image-updater (the latter deliberately rejected: they drop the human review gate that caught the authelia /data + stremthru pgdata incidents).
