...
# Per-pod memory usage for the sizing checks. Prefer VM PEAK (max working_set over a
...
# Caveat: peak is per exact pod name, so a freshly-rolled pod under-reports until it
# accumulates history — conservative (won't false-positive), self-corrects over time.
USAGE_SRC="none"
...
if usage_src == "vm-peak":
    ...
    for r in vm.get("data", {}).get("result", []):
        ...
        if mi > usage.get((ns, name), 0.0): usage[(ns, name)] = mi
    usage_label = f"{os.environ.get('VM_PEAK_WINDOW', '30d')} peak"
else:
...
for pod in pods:
    ...
    if sum_lim >= OVERSIZE_FLOOR_MI and sum_lim >= OVERSIZE_RATIO * max(live, 1):
        print(f"LOW|resource/oversized-limit|{ns}/{name} memory limit {fmt(sum_lim)} is {sum_lim/max(live,1):.0f}× {usage_label} {fmt(live)} — reclaim headroom, lowers overcommit|right-size limits.memory toward ~2× steady-state in its manifest")
    # 3. under-requested
    if sum_req > 0 and live >= UNDERREQ_FLOOR_MI and live >= UNDERREQ_RATIO * sum_req:
        print(f"MED|resource/under-request|{ns}/{name} {usage_label} {fmt(live)} vs request {fmt(sum_req)} ({live/sum_req:.1f}× over request) — scheduler undercounts real demand|raise requests.memory toward observed usage in its manifest")
PY
