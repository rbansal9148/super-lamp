...
# Built by gitops/scripts/update-avs.sh.
...
# Only HOME is baked here. PORT/REFRESH_MINUTES/API_CONCURRENCY are left to the app's own code
# defaults (8080 / 1440 / 1 — src/server.ts, src/limit.ts); baking stale values only forced the
# k8s manifest to override them back.
ENV HOME=/tmp
EXPOSE 8080
